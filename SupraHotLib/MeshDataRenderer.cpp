...
namespace SupraHot
{
	...
	void MeshDataRenderer::Render(Graphics::Camera* camera)
	{
		...
		for (MeshComponent* meshComponent : MeshComponents)
		{
			...
			if (meshData->Material.GetAlbedoMap() != nullptr)
			{
				shader->SetTexture2D(glGetUniformLocation(shaderProgramID, "AlbedoTexture"), meshData->Material.GetAlbedoMap(), GL_TEXTURE0);
			}
